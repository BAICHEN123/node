...
{
	detachInterrupt(digitalPinToInterrupt(pin_key_1));
	detachInterrupt(digitalPinToInterrupt(pin_key_2));
}
void stop_mada_do()
{
	mada1 = 0;
	mada2 = 0;
	mada_stop(&mada);
	clean_key_down_Interrupt();
}
ICACHE_RAM_ATTR void key_down_Interrupt()
{
	if (mada.type != TYPE_MADA_WAIT && mada.type != TYPE_MADA_STOP && (digitalRead(pin_key_1) == LOW || digitalRead(pin_key_2) == LOW))
	{
		stop_mada_do();
	}
}
