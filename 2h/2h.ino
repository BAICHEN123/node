...
3	添加低功耗状态，允许用户选择设备是否开启低功耗状态。
	...
	重复上一步骤
4	是否可以强制唤醒？
5	这里我为了移植方便把定时中断关闭了，导致无法清除用户记录的wifi，只能在刚刚复位链接WiFi的时候进行清除
*/
/*
//为数据命名，设定个数
...
void setup()
{
	...
	digitalWrite(LED_BUILTIN, HIGH); //不知道为啥，这个模块的初始状态是开灯
	//pinMode(16, OUTPUT);
	//digitalWrite(LED_BUILTIN, HIGH); //不知道为啥，这个模块的初始状态是LOW，然后我一插上跳线帽就开始无限重启//是电压低的问题，默认未初始化的电压低于判定电压，在 nodemcu 上没有出现错误可能是因为产品型号/批次的不同，经过电压表测量，nodemcu的电压在0.8V左右，单个小模块的电压不到0.3
